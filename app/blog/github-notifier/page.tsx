...
import '../todo-project/blog-styles.css';

export default function GitHubNotifierBlog() {
  const markdownPath = path.join(process.cwd(), 'app', 'blog', 'github-notifier', 'blog_post.md');
  const markdownContent = fs.readFileSync(markdownPath, 'utf-8');

  return (
    <div className="min-h-screen w-full overflow-x-hidden bg-zinc-50 font-sans dark:bg-black">
      {/* Navbar */}
      <nav className="fixed top-0 left-0 right-0 z-50 bg-white dark:bg-black border-b border-black/[.08] dark:border-white/[.145]">
        <div className="flex items-center justify-between px-6 py-4 max-w-3xl mx-auto">
          ...
          <div className="flex items-center gap-3">
            <a
              href="https://github.com/conradtondryk/github-bot"
              target="_blank"
              rel="noopener noreferrer"
              className="inline-flex items-center gap-2 px-4 py-2 rounded-lg bg-black dark:bg-white text-white dark:text-black hover:bg-zinc-800 dark:hover:bg-zinc-200 transition-colors text-sm font-medium"
            >
              <svg
                className="w-5 h-5"
                fill="currentColor"
      ...
      <main className="flex min-h-screen w-full max-w-3xl flex-col pt-24 pb-32 px-6 sm:px-16 bg-white dark:bg-black mx-auto">
        <div className="w-full border-t border-black/[.08] dark:border-white/[.145] mb-8"></div>
